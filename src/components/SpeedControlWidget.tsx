...
export function SpeedControlWidget({
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="fixed bottom-20 left-3 z-40 flex items-center gap-1.5 px-3 py-2 rounded-full bg-primary text-primary-foreground shadow-lg hover:shadow-xl transition-all text-xs font-arabic"
        title={label}
      >
  ...
  return (
    <div className="fixed bottom-20 left-3 z-40 bg-card border border-border rounded-2xl shadow-2xl p-4 w-56 space-y-3 animate-fade-in" dir="rtl">
      <div className="flex items-center justify-between">
        <span className="text-xs font-arabic font-bold text-foreground">{label}</span>
        <button onClick={() => setExpanded(false)} className="w-6 h-6 rounded-full flex items-center justify-center hover:bg-muted transition-colors">
